...
var drag_start_global_pos = null
var max_movement_for_act_independently = 10 #If an object is released more than this distance fron where it started being dragged (line above), it will only interact, not act independently.
@onready var default_mode = mode
@onready var default_z_index = z_index
@onready var default_z_as_relative = z_as_relative
...
func _ready():
	...
	if not Engine.is_editor_hint():
		...
		if len(DisplayName) > 1:
			var tooltip_container = Node2D.new()
...
func _process(delta):
	#tooltip visibility
	if tooltip:
		tooltip.visible = (GameSettings.objectTooltips and global_position.distance_squared_to(get_global_mouse_position()) < pow(tooltip_display_distance, 2))
	...
	if dragging:
		...
		if can_change_subscenes:
			...
			var desired_subscene = get_node("/root/Main").GetDeepestSubsceneAt(global_position)
			if GetSubsceneManagerParent() != desired_subscene:
				if desired_subscene == null:
					#we're in a subscene, but need to not be in any (null)
					var new_parent = GetSubsceneManagerParent().get_parent()
...
func ClampObjectPosition() -> void:
	var current_module = GetCurrentModuleScene()
	var lab_boundary = ""
	for child in current_module.get_children():
		if child.name == "LabBoundary":
			lab_boundary = child
	if typeof(lab_boundary) != TYPE_STRING:
...
func GetIntersectingLabObjects():
	...
	for child in get_children():
		if child is CollisionShape2D:
			...
			query_options.set_shape(child.shape)
			query_options.transform = child.get_global_transform()
			query_options.exclude = [self]
			query_options.collision_layer = 2 #the objects layer and no others
...
func OnUserAction():
	...
	if interact_options:
		...
		var result = self.TryInteract(interact_options)
