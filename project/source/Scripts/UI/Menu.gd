...
@onready var _prompt_panel_stylebox_allowed: StyleBox = $%PromptTemplate.get_theme_stylebox("panel").duplicate()
@onready var _prompt_panel_stylebox_disallowed: StyleBox = _prompt_panel_stylebox_allowed.duplicate()
var _interaction_prompts: Dictionary[InteractInfo.Kind, PanelContainer] = {}
...
func _ready() -> void:
	...
	# Make prompts for every interaction.
	for k: InteractInfo.Kind in InteractInfo.Kind.values():
		var c: PanelContainer = $%PromptTemplate.duplicate()
		$%PromptTemplate.get_parent().add_child(c)
		_interaction_prompts.set(k, c)
...
func _process(delta: float) -> void:
	...
	for kind: InteractInfo.Kind in _interaction_prompts.keys():
		var prompt_panel: PanelContainer = _interaction_prompts.get(kind)
