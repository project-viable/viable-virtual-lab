...
func start_use(_area: InteractableArea, _kind: InteractInfo.Kind, event: InputEvent = null) -> void:
	if not (_area is ScoopInteractableArea):
		return
	match _kind:
		0:
			...
			if not _area.container.substances.is_empty():
				if vol_to_take > _area.container.get_total_volume() or vol_to_take > container.get_volume():
					print("too many ml added to scoopula")
				else:
					### Add contents to scoopula and update containers's substance volume
					container.add(_area.container.take_volume(vol_to_take))
		...
		1:
			if container.substances.is_empty():
				...
				## Reset scoopula
				if not _area.container.substances.is_empty():
					_area.container.add(container.take_volume(container.get_volume()-_area.container.get_total_volume()))
				else:
					_area.container.add(container.take_volume(container.get_volume()-0.0))
					container.substances.clear()
					get_parent().find_child("FillSprite").visible = false
					#print(_area.container, " after being dumped into has volume of: ", _area.container.get_total_volume())
