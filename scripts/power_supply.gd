...
const MAX_INPUT_TIME: float = 99 * 60 + 59
...
func _ready() -> void:
	_update_display()
	_set_timer_enabled(_is_timer_enabled)
...
func _update_display() -> void:
	%TimeDisplay/Colon.visible = _is_timer_enabled
	if _is_timer_enabled:
		# Divide by 60 to get minutes.
		var time_to_show: float = $LabTimer.time_left / 60.0 if _is_outputting else _input_time_mins
...
func _set_timer_enabled(enabled: bool) -> void:
	_is_timer_enabled = enabled
...
func _update_timer_pause() -> void:
	$LabTimer.paused = not _is_outputting or not _is_timer_enabled
...
func _on_power_button_pressed() -> void:
	...
	# Turn off timer when turning off output, and start running it when turning it on.
	if not _is_outputting:
		_set_timer_enabled(false)
	elif _is_timer_enabled:
...
func _on_time_toggle_button_pressed() -> void:
	# We can freely enable the timer when output is off, and we can always disable the timer while
	# it's running, but we can't enable the timer if it's off while running.
	if not _is_outputting or _is_timer_enabled:
		_set_timer_enabled(not _is_timer_enabled)

func _on_time_up_button_started_holding() -> void:
	# You can't edit the time while output is running or while the timer is off.
	if _is_outputting or not _is_timer_enabled: return
